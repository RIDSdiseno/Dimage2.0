<template>
    <AppLayout :title="`${tipo.label_plural} - Admin`">
        <div class="p-6">
            ...
            <div class="flex items-center justify-between mb-6">
                <div>
                    <div class="flex items-center gap-2 mb-1">
                        ...
                        <span class="text-sm text-gray-600">{{ tipo.label_plural }}</span>
                    </div>
                    <h1 class="text-2xl font-bold text-gray-800">{{ tipo.label_plural }}</h1>
                    <p class="text-sm text-gray-400 mt-0.5">{{ total }} encontrados</p>
                </div>
                <Link :href="route(tipo.route_create)">
                    <Button :label="`Crear ${tipo.label}`" icon="pi pi-plus"
            ...
            <div class="bg-white p-4 rounded-xl border border-gray-100 shadow-sm mb-5 flex flex-wrap gap-3 items-end">
                <div class="flex-1 min-w-48">
                    ...
                    <IconField>
                        ...
                        <InputText v-model="filters.search" placeholder="Nombre, RUT o username..." class="w-full" @input="onSearch" />
                    </IconField>
                </div>
            </div>
                ...
                <table class="w-full text-sm">
                    <thead style="background-color:#f8fafc;">
                        <tr>
                            ...
                            <th class="text-left px-5 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wide hidden lg:table-cell">Clínicas</th>
                    ...
                    <tbody>
                        <template v-if="loading">
                            <tr v-for="n in 8" :key="n" class="border-t border-gray-50 animate-pulse">
                                ...
                                <td class="px-5 py-3.5 hidden lg:table-cell"><div class="h-3 bg-gray-100 rounded w-24" /></td>
                                <td class="px-5 py-3.5"><div class="h-5 bg-gray-100 rounded-full w-16" /></td>
                                <td class="px-5 py-3.5"></td>
                            </tr>
                        </template>
                        <template v-else-if="staff.length">
                            <tr v-for="s in staff" :key="s.id" class="border-t border-gray-50 hover:bg-blue-50/30 transition">
                                <td class="px-5 py-3.5 font-medium text-gray-800">{{ s.name }}</td>
                                <td class="px-5 py-3.5 text-gray-500 hidden md:table-cell font-mono text-xs">{{ s.username }}</td>
                                <td class="px-5 py-3.5 text-gray-500 hidden md:table-cell">{{ s.rut || '-' }}</td>
                                <td class="px-5 py-3.5 text-gray-500 hidden lg:table-cell">{{ s.mail || '-' }}</td>
                                <td class="px-5 py-3.5 hidden lg:table-cell">
                                    <span v-if="s.clinicas?.length" class="text-xs text-gray-600">
                                        {{ s.clinicas.join(', ') }}
                                    </span>
                                    <span v-else class="text-xs text-gray-400 italic">Sin clínicas</span>
                                </td>
                                <td class="px-5 py-3.5">
                                    <span :class="s.activo ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-600'"
                                ...
                                <td class="px-5 py-3.5 text-right">
                                    <div class="flex items-center justify-end gap-1">
                                        <Link :href="route(tipo.route_edit, s.id)">
                                            <Button icon="pi pi-pencil" size="small" text v-tooltip.top="'Editar'" />
                                        </Link>
                                        <Button
                                            :icon="s.activo ? 'pi pi-eye-slash' : 'pi pi-eye'"
                                            size="small" text
                                            :severity="s.activo ? 'warning' : 'success'"
                                            :v-tooltip.top="s.activo ? 'Desactivar' : 'Activar'"
                        ...
                        <tr v-else>
                            <td colspan="7" class="px-5 py-16 text-center">
                                ...
                                <p class="text-gray-400 font-medium">No se encontraron {{ tipo.label_plural.toLowerCase() }}.</p>
                            </td>
                        </tr>
                    </tbody>
                </table>
...
const props = defineProps({
    ...
    perPage:     Number,
    filters:     Object,
    tipo:        Object, // { label, label_plural, route_index, route_create, route_edit, route_toggle }
});
...
const applyFilters = (page = 1) => {
    loading.value = true;
    router.get(route(props.tipo.route_index), { search: filters.value.search, page }, {
...
const toggleStaff = (id) => {
    router.post(route(props.tipo.route_toggle, id), {}, { preserveScroll: true });
};
</script>
